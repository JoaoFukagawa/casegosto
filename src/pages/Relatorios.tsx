...
import { CalendarDays, Download, TrendingDown, FileBarChart, Package, TrendingUp, Trophy, BarChart3 } from "lucide-react";
...
export default function Relatorios() {
  ...
  type ProdAgg = {
    ...
    entries: number; // nº de lançamentos
    revenue: number;
    minPrice: number;
    maxPrice: number;
    prices: number[];
  ...
  const salesStats = useMemo(() => {
    ...
    // estatísticas (sobre receita por produto)
    const revs = productSales.map((p) => p.revenue).sort((a, b) => a - b);
    const median = revs.length
      ? revs.length % 2
        ? revs[(revs.length - 1) / 2]
        : (revs[revs.length / 2 - 1] + revs[revs.length / 2]) / 2
      : 0;
    ...
    return { totalRevenue, totalUnits, totalKg, totalEntries, distinct, top, ordersCount, avgTicketOrder, avgPerEntry, median, stdDev };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Análise detalhada de despesas por período e categoria
          </p>
        </div>
      ...
      <div className="space-y-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="font-heading text-lg">Detalhamento por produto</CardTitle>
          </CardHeader>
          <CardContent>
            {loadingSales ? (
                      ...
                      return (
                        <TableRow key={p.menu_item_id}>
                          ...
                          <TableCell className="text-right text-xs text-muted-foreground">
                            R$ {minP.toFixed(2)} / R$ {p.maxPrice.toFixed(2)}
                          </TableCell>
      ...
      {Object.keys(byCategory).length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="font-heading text-lg">Por Categoria</CardTitle>
          </CardHeader>
          <CardContent>
